...
fn resolve_llama_src(manifest_dir: &str) -> PathBuf {
    let manifest_dir = Path::new(manifest_dir);

    if let Ok(p) = env::var("LLAMA_CPP_SRC") {
        let p = PathBuf::from(p);
        let p = if p.is_relative() { manifest_dir.join(p) } else { p };
        if p.join("CMakeLists.txt").exists() {
            return p;
        }
        panic!(
            "LLAMA_CPP_SRC is set but invalid (missing CMakeLists.txt): {}",
            p.display()
        );
    }

    let bundled = manifest_dir.join("llama.cpp");
    if bundled.join("CMakeLists.txt").exists() {
        return bundled;
    }

    // Repo-local fallback: this workspace vendors a full llama.cpp tree at repo root.
    let repo_fallback = manifest_dir.join("..").join("..").join("llama.cpp-b7772");
    if repo_fallback.join("CMakeLists.txt").exists() {
...
Expected one of:\n\
- $CARGO_MANIFEST_DIR/llama.cpp (git submodule)\n\
- ../../llama.cpp-b7772 (repo vendored)\n\
Or set LLAMA_CPP_SRC to a llama.cpp source directory.\n\
Searched from: {}",
...
fn maybe_set_libclang_path() {
    ...
    // Typical Visual Studio LLVM toolset locations (clang.dll / libclang.dll).
    let candidates = [
        ...
        r"C:\Program Files\Microsoft Visual Studio\2022\BuildTools\VC\Tools\Llvm\bin",
    ];
    for p in candidates {
        let dir = Path::new(p);
        if dir.join("libclang.dll").exists() || dir.join("clang.dll").exists() {
            env::set_var("LIBCLANG_PATH", p);
            println!("cargo:warning=auto-set LIBCLANG_PATH={p}");
            return;
        }
    }
}

fn maybe_force_cuda_13_windows() {
    if !cfg!(all(target_os = "windows", feature = "cuda")) {
        return;
    }

    let desired = Path::new(r"C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v13.0");
    let desired_ok = desired.join("lib").join("x64").join("cublas.lib").exists();
    if !desired_ok {
        return;
    }

    let current = env::var_os("CUDA_PATH").map(PathBuf::from);
    let current_ok = current
        .as_ref()
        .map(|p| p.join("lib").join("x64").join("cublas.lib").exists())
        .unwrap_or(false);
    if current_ok {
        return;
    }

    env::set_var("CUDA_PATH", desired);
    env::set_var("CUDAToolkit_ROOT", desired);
    println!("cargo:warning=forcing CUDA_PATH to {}", desired.display());
}
...
fn main() {
    ...
    let llama_src = resolve_llama_src(&manifest_dir);
